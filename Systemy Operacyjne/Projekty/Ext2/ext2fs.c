...
int ext2_block_used(uint32_t blkaddr) {
  ...
  /* TODO */
  --blkaddr;
  uint32_t nr_of_group = blkaddr / blocks_per_group;
  ext2_groupdesc_t gd = group_desc[nr_of_group];
  blk_t *blk = blk_get(0, gd.gd_b_bitmap);
  uint8_t *bitmap_byte = blk->b_data;
  uint32_t nr_of_blk = blkaddr % blocks_per_group;
  if (*(bitmap_byte + (nr_of_blk / 8)) & 1 << (nr_of_blk % 8)) {
    used = 1;
  }
  blk_put(blk);
  /* TODO DONE */
...
int ext2_inode_used(uint32_t ino) {
  ...
  /* TODO */
  --ino;
  uint32_t nr_of_group = ino / inodes_per_group;
  ext2_groupdesc_t gd = group_desc[nr_of_group];
  blk_t *blk = blk_get(0, gd.gd_i_bitmap);
  uint8_t *bitmap_byte = blk->b_data;
  uint32_t idx_of_ino = ino % inodes_per_group;
  if (*(bitmap_byte + (idx_of_ino / 8)) & 1 << (idx_of_ino % 8)) {
    used = 1;
  }
  blk_put(blk);
  /* TODO DONE */
...
static int ext2_inode_read(off_t ino, ext2_inode_t *inode) {
  ...
  if (ext2_inode_used(ino) == 1) {
    --ino;
    uint32_t nr_of_group = ino / inodes_per_group;
    uint32_t i_tables = group_desc[nr_of_group].gd_i_tables;
    uint32_t idx_of_ino = ino % inodes_per_group;
    size_t pos = (i_tables + (idx_of_ino / BLK_INODES)) * BLKSIZE +
                 (sizeof(ext2_inode_t) * (idx_of_ino % BLK_INODES));
    if (!ext2_read(0, inode, pos, sizeof(ext2_inode_t))) {
      return 0;
    }
  }
  /* TODO DONE */
...
static uint32_t ext2_blkptr_read(uint32_t blkaddr, uint32_t blkidx) {
  ...
  uint32_t *b_data = blk->b_data;
  blk_put(blk);
  return b_data[blkidx];
  /* TODO DONE */
}
...
long ext2_blkaddr_read(uint32_t ino, uint32_t blkidx) {
  ...
  if (blkidx < 12 + BLK_POINTERS) {
    return ext2_blkptr_read(inode.i_blocks[12], blkidx - 12);
  }
  uint32_t blk = 0;
  if (blkidx < 12 + BLK_POINTERS + (BLK_POINTERS * BLK_POINTERS)) {
    blk = ext2_blkptr_read(inode.i_blocks[13],
                           (blkidx - (12 + BLK_POINTERS)) / BLK_POINTERS);
    return ext2_blkptr_read(blk, (blkidx - (12 + BLK_POINTERS)) % BLK_POINTERS);
  }
  if (blkidx < 12 + BLK_POINTERS + (BLK_POINTERS * BLK_POINTERS) +
                 (BLK_POINTERS * BLK_POINTERS * BLK_POINTERS)) {
    blk = ext2_blkptr_read(inode.i_blocks[14],
                           (blkidx - (12 + (BLK_POINTERS * BLK_POINTERS))) /
                             BLK_POINTERS);
    blk = ext2_blkptr_read(
      blk, (blkidx - (12 + (BLK_POINTERS * BLK_POINTERS))) / BLK_POINTERS);
...
int ext2_read(uint32_t ino, void *data, size_t pos, size_t len) {
  ...
  if (ino) {
    ext2_inode_t inode;
    if (ext2_inode_read(ino, &inode) == ENOENT || inode.i_size < pos + len) {
      return EINVAL;
    }
  }
  uint32_t idx = pos / BLKSIZE;
  blk_t *blk = NULL;
  for (uint32_t bytes_read = 0; bytes_read < len; idx++) {
    blk = blk_get(ino, idx);
    uint32_t offset = 0;
    if (bytes_read == 0) {
      offset = pos % BLKSIZE;
    }
    uint32_t bytes_to_read = min(BLKSIZE - offset, len);
    if (blk == BLK_ZERO) {
      memset(data, 0, bytes_to_read);
    } else {
      memcpy(data, blk->b_data + offset, bytes_to_read);
      blk_put(blk);
    }
    data += bytes_to_read;
...
int ext2_readdir(uint32_t ino, uint32_t *off_p, ext2_dirent_t *de) {
  ...
  bool inode_not_valid = 0;
  while (!inode_not_valid) {
    if (inode.i_size <= *off_p) {
      return 0;
    }
    ext2_read(ino, de, *off_p, de_name_offset);
    inode_not_valid = de->de_ino;
...
int ext2_mount(const char *fspath) {
  ...
  if (!(ext2_read(0, group_desc, 2 * BLKSIZE,
