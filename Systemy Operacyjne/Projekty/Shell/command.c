...
noreturn void external_command(char **argv) {
  ...
  if (!index(argv[0], '/') && path) {
    ...
    while (*path) {
      size_t n = strcspn(path, ":"); // szukamy pierwszego wysąpienia ':'

      char *dir =
        strndup(path, n); // kopiuje fragment zmiennej path do pierwszego
                          // wystąpienia ':' (lub całość, jeżeli nie występuje)
      strapp(&dir, "/");
      strapp(&dir, argv[0]); // dokleja nazwę programu
      (void)execve(dir, argv, environ);
      path +=
        n + (n != strlen(path)); // przesuwa wskaźnik na kolejny element z path
      free(dir);                 // czyszczenie dir
    }
    /* TODO END */
  } else {
